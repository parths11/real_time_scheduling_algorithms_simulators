	...
	while(timer<=simulation_time)
	  {
			 ...
			 if(t[active_task_id].T[critical_start]==1 && t[active_task_id].critical_execution_copy==t[active_task_id].critical_execution)
			 {
			 	if(r[t[active_task_id].T[resource_used]].lock==0)
				{
					...
					if(nx==res || yeah==ax)
					{
						...
						if(t[active_task_id].critical_execution_copy==t[active_task_id].critical_execution)
							fprintf(opfiles[active_task_id],"%f, %d, self\n",timer,t[active_task_id].T[priority]);
					}
					else
					{
				...
				{
					o=0;
					o=t[r[t[active_task_id].T[resource_used]].task_exe].T[priority];
					t[r[t[active_task_id].T[resource_used]].task_exe].T[priority] = t[active_task_id].T[priority];
					if(o!=t[r[t[active_task_id].T[resource_used]].task_exe].T[priority])
					{
						fprintf(opfiles[r[t[active_task_id].T[resource_used]].task_exe],"%f, %d, %d\n",timer,t[active_task_id].T[priority],active_task_id);
					}
					if(o!=t[r[t[active_task_id].T[resource_used]].task_exe].T[priority_copy])
					{
						t[tasks[o]].T[priority]=o;
					}
					t[active_task_id].T[priority] = t[active_task_id].T[priority]+n;
					active_task_id=min(t,n,priority);
				}
			 }
