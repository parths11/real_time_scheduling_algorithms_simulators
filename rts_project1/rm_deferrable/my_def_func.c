...
int aperiodic_task_input(aperiodic *a1,int n, FILE* fp)
{
	int result=0;
	char line[1025];
	char value[10];
...
value[u]='\0';
a1->A[ap_execution]=atoi(value);
success++;
i++;}

if (success==5)
{
	a1->A[execution_copy]=a1->A[ap_execution];
	a1->alive = 0;
	a1++;
	break;
}
...
int min_aperiodic(aperiodic *a1,int n,int p)
{
	int i=0,min=0x7FFF,task_id=IDLE_TASK_ID;
	while(i<n)
	{
		if(min>a1->A[p] && a1->alive==1)
		{
			min=a1->A[p];
			task_id=i;
		}
...
int aperiodic_interrupt(aperiodic *a1,int tmr,int n)
{
int i=0,n1=0,a=0;
aperiodic *a1_copy;
a1_copy=a1;
	while(i<n)
	{
		if(tmr >= a1->A[ap_arrival] && a1->A[ap_execution]!=0)
			{
			a1->alive = 1;
			a++;
			}
